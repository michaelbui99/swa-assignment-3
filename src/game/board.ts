    ...
    function gameLoop(board: Board<T>, effects: Effect<T>[]): MoveResult<T> {
        ...
        if (allMatches.length > 0) {
            ...
            if (refillResult.hasRefilled) {
                ...
                return gameLoop(refillResult.board, [
                    ...effects,
...
function refillEmptyPostions<T>(
    board: Board<T>,
    generator: Generator<T>
): RefillResult<T> {
    const emptyPositions = getEmptyPositions(board);
    const refilledBoard = emptyPositions.reduce(
        (currentBoardState, currentEmptyPosition) =>
            setPiece(currentBoardState, currentEmptyPosition, generator.next()),
        board
    );
    return { board: refilledBoard, hasRefilled: emptyPositions.length > 0 };
}

function clearMatches<T>(
    board: Board<T>,
    matchesToClear: Match<T>[]
): Board<T> {
    return matchesToClear.reduce((_, currentMatch) => {
        return currentMatch.positions.reduce(
            (boardAccumulator, currentPosition) => {
...
function movePiecesInColumnToButtom<T>(
    board: Board<T>,
    position: Position
): Board<T> {
    if (position.row < 0) {
        ...
        return movePiecesInColumnToButtom(newBoardState, {
            row: position.row - 1,
            col: position.col,
        });
    }
...
function movePieceToButtom<T>(board: Board<T>, position: Position): Board<T> {
    ...
    if (belowPiece !== undefined) {
        ...
        if (moveResult.stateChanged) {
            return movePieceToButtom(
                moveResult.board,
                moveResult.newPiecePosition
            );
        } else {
...
function movePiecesDown<T>(board: Board<T>): Board<T> {
    function do_movePiecesDown<T>(
        currentBoardState: Board<T>,
        currentColumn: number
    ): Board<T> {
        if (currentColumn >= currentBoardState.width) {
            return currentBoardState;
        } else {
            const newBoardState = movePiecesInColumnToButtom(
                currentBoardState,
                { row: currentBoardState.height - 1, col: currentColumn }
            );
...
function getAllMatches<T>(board: Board<T>): Match<T>[] {
    function collectHorizontalMatches(
        ...
        if (isOutsideBoard(board, currentPosition)) {
            ...
            return collectHorizontalMatches(board, newPosition, [
                ...allMatches,
                ...horizontalMatches,
            ]);
        }
    ...
    function collectVerticalMatches(
        ...
        if (isOutsideBoard(board, currentPosition)) {
            ...
            return collectVerticalMatches(board, newPosition, [
                ...allMatches,
                ...verticalMatches,
            ]);
        }
    ...
    const verticalMatches = collectVerticalMatches(
        board,
        { row: 0, col: 0 },
        []
    );
    const horizontalMatches = collectHorizontalMatches(
        board,
        { row: 0, col: 0 },
        []
    );
